...
class WalletSendTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        for mode in ["unset", "economical", "conservative", "chi/kb", "sat/b"]:
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0.00007, estimate_mode="chi/kb", add_to_wallet=False)
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=1, arg_conf_target=0.00004531, arg_estimate_mode="chi/kb", add_to_wallet=False)
        fee = self.nodes[1].decodepsbt(res["psbt"])["fee"]
        ...
        for mode in ["chi/kb", "sat/b"]:
            ...
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=0, estimate_mode=mode,
                expect_error=(-4, "Fee rate (0.00000000 CHI/kB) is lower than the minimum fee rate setting (0.00001000 CHI/kB)"))
        ...
        # TODO: error should use sat/B instead of CHI/kB if sat/B is selected.
        # Test setting explicit fee rate just below the minimum.
        for unit, fee_rate in {"sat/B": 0.99999999, "CHI/kB": 0.00000999}.items():
            ...
            self.test_send(from_wallet=w0, to_wallet=w1, amount=1, conf_target=fee_rate, estimate_mode=unit,
                expect_error=(-4, "Fee rate (0.00000999 CHI/kB) is lower than the minimum fee rate setting (0.00001000 CHI/kB)"))

        self.log.info("Explicit fee rate raises RPC error if estimate_mode is passed without a conf_target")
        for unit, fee_rate in {"sat/B": 100, "CHI/kB": 0.001}.items():
