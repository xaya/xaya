...
static void mineBlock(const node::NodeContext& node, std::chrono::seconds block_time)
{
    ...
    CBlock block = node::BlockAssembler{node.chainman->ActiveChainstate(), nullptr, {}}.CreateNewBlock(PowAlgo::NEOSCRYPT)->block;
...
BOOST_AUTO_TEST_CASE(connections_desirable_service_flags)
{
    ...
    const auto spacing = std::chrono::seconds{AvgTargetSpacing(m_node.chainman->GetConsensus(), m_node.chainman->ActiveChain().Height())};
    SetMockTime(GetTime<std::chrono::seconds>() + std::chrono::seconds{spacing * (NODE_NETWORK_LIMITED_ALLOW_CONN_BLOCKS - 1)});
    BOOST_CHECK(peerman->GetDesirableServiceFlags(peer_flags) == ServiceFlags(NODE_NETWORK_LIMITED | NODE_WITNESS));

    // Check we disallow limited peers connections when we are further than the limited peers safety window
    SetMockTime(GetTime<std::chrono::seconds>() + std::chrono::seconds{spacing * 2});
    BOOST_CHECK(peerman->GetDesirableServiceFlags(peer_flags) == ServiceFlags(NODE_NETWORK | NODE_WITNESS));
    ...
    SetMockTime(GetTime<std::chrono::seconds>() + std::chrono::seconds{spacing.count() * NODE_NETWORK_LIMITED_ALLOW_CONN_BLOCKS + 1});
    BOOST_CHECK(peerman->GetDesirableServiceFlags(peer_flags) == ServiceFlags(NODE_NETWORK | NODE_WITNESS));
}
