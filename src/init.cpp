...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    g_zmq_notification_interface = CZMQNotificationInterface::Create(
        ...
        [&chainman = node.chainman](const uint256& hash) {
            assert(chainman);
    ...
    if (gArgs.GetBoolArg("-namehashindex", DEFAULT_NAMEHASHINDEX)) {
        g_name_hash_index = std::make_unique<NameHashIndex>(interfaces::MakeChain(node), cache_sizes.name_hash_index, false, chainman.m_blockman.m_reindexing);
    ...
    g_send_updates_worker.reset(new SendUpdatesWorker (chainman.m_blockman));
