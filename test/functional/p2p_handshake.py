...
class P2PHandshakeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # In Xaya, we have a block every 30 seconds, so the window to
        # consider there is 144 * 30 = 4320 seconds.
        self.generate_at_mocktime(int(time.time()) - 4400)  # tip outside the 24h window, should fail
        self.test_desirable_service_flags(node, [NODE_NETWORK_LIMITED | NODE_WITNESS],
                                          DESIRABLE_SERVICE_FLAGS_FULL, expect_disconnect=True)
        self.generate_at_mocktime(int(time.time()) - 4300)  # tip inside the 24h window, should succeed
        self.test_desirable_service_flags(node, [NODE_NETWORK_LIMITED | NODE_WITNESS],
