...
class PSBTTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test walletcreatefundedpsbt feeRate of 0.1 NMC/kB produces a total fee at or slightly below -maxtxfee (~0.05290000)")
        ...
        for unit, fee_rate in {"nmc/kb": 0.1, "sat/b": 10000}.items():
        ...
        for mode in ["unset", "economical", "conservative", "nmc/kb", "sat/b"]:
            ...
            if mode in ["nmc/kb", "sat/b"]:
                assert_raises_rpc_error(-3, "Amount out of range",
                    lambda: self.nodes[1].walletcreatefundedpsbt(inputs, outputs, 0, {"estimate_mode": mode, "conf_target": -1, "add_inputs": True}))
                assert_raises_rpc_error(-4, "Fee rate (0.00000000 NMC/kB) is lower than the minimum fee rate setting (0.00001000 NMC/kB)",
        ...
        for unit, fee_rate in {"SAT/B": 0.99999999, "NMC/KB": 0.00000999}.items():
            ...
            assert_raises_rpc_error(-4, "Fee rate (0.00000999 NMC/kB) is lower than the minimum fee rate setting (0.00001000 NMC/kB)",
                lambda: self.nodes[1].walletcreatefundedpsbt(inputs, outputs, 0, {"estimate_mode": unit, "conf_target": fee_rate, "add_inputs": True}))

        self.log.info("Test walletcreatefundedpsbt feeRate of 10 NMC/kB produces total fee well above -maxtxfee and raises RPC error")
