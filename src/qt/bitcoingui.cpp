...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Xaya address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments and name transfers (generates QR codes and xaya: URIs)"));
    ...
    buyNamesAction = new QAction(platformStyle->SingleColorIcon(":/icons/manage-names"), tr("&Buy Names"), this);
    ...
    openAction->setStatusTip(tr("Open a xaya: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Xaya command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Xaya network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void BitcoinGUI::updateHeadersSyncProgressLabel()
{
    ...
    int estHeadersLeft = (GetTime() - headersTipTime) / Ticks<std::chrono::seconds>(AvgTargetSpacing(Params().GetConsensus(), headersTipHeight));
...
{
    int estHeadersLeft = blockDate.secsTo(QDateTime::currentDateTime()) / Ticks<std::chrono::seconds>(AvgTargetSpacing(Params().GetConsensus(), height));
    if (estHeadersLeft > HEADER_HEIGHT_DELTA_SYNC)
