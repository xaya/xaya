...
public:
    ...
    std::unique_ptr<BlockTemplate> createNewBlock(const PowAlgo algo, const BlockCreateOptions& options) override
    {
        ...
        return std::make_unique<BlockTemplateImpl>(assemble_options, BlockAssembler{chainman().ActiveChainstate(), context()->mempool.get(), assemble_options}.CreateNewBlock(algo), m_node);
    }

    bool checkBlock(const CBlock& block, const node::BlockCheckOptions& options, std::string& reason, std::string& debug) override
    {
        LOCK(chainman().GetMutex());
        BlockValidationState state{TestBlockValidity(chainman().ActiveChainstate(), block, /*check_pow=*/options.check_pow, /*check_bits=*/true, /*=check_merkle_root=*/options.check_merkle_root)};
