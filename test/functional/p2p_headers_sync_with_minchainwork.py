...
class RejectLowDifficultyHeadersTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        work1 = (NODE1_BLOCKS_REQUIRED + 1) * 2_048
        work2 = (NODE2_BLOCKS_REQUIRED + 1) * 2_048
        self.extra_args = [
            ["-minimumchainwork=0x0", "-checkblockindex=0"],
            ["-minimumchainwork=%x" % work1, "-checkblockindex=0"],
            ["-minimumchainwork=%x" % work2, "-checkblockindex=0"],
            ["-minimumchainwork=%x" % work2, "-checkblockindex=0", "-whitelist=noban@127.0.0.1"],
        ]
        for a in self.extra_args:
            a.append("-checknamedb=-1")
    ...
    def test_chains_sync_when_long_enough(self):
        ...
        for node in self.nodes[1:3]:
            ...
            assert {
                'height': 0,
                'hash': '6f750b36d22f1dc3d0a6e483af45301022646dfc3b3ba2187865f5a7d6d83ab1',
                'branchlen': 0,
                'status': 'active',
            } in chaintips
    ...
    def test_large_reorgs_can_succeed(self):
        # Disabled on Xaya, as it seems flaky and takes a long
        # time due to neoscrypt.
        return
