...
def setup():
    ...
    if not os.path.isdir('namecoin-core'):
        subprocess.check_call(['git', 'clone', 'https://github.com/namecoin/namecoin-core.git'])
...
def build():
    ...
    os.makedirs('namecoin-binaries/' + args.version, exist_ok=True)
    ...
    if args.linux:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'namecoin='+args.commit, '--url', 'namecoin='+args.url, '../namecoin-core/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call(['bin/gsign', '-p', args.sign_prog, '--signer', args.signer, '--release', args.version+'-linux', '--destination', '../gitian.sigs/', '../namecoin-core/contrib/gitian-descriptors/gitian-linux.yml'])
        subprocess.check_call('mv build/out/namecoin-*.tar.gz build/out/src/namecoin-*.tar.gz ../namecoin-binaries/'+args.version, shell=True)
...
def verify():
    ...
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-linux', '../namecoin-core/contrib/gitian-descriptors/gitian-linux.yml'])
    print('\nVerifying v'+args.version+' Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-unsigned', '../namecoin-core/contrib/gitian-descriptors/gitian-win.yml'])
    print('\nVerifying v'+args.version+' MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-unsigned', '../namecoin-core/contrib/gitian-descriptors/gitian-osx.yml'])
    print('\nVerifying v'+args.version+' Signed Windows\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-win-signed', '../namecoin-core/contrib/gitian-descriptors/gitian-win-signer.yml'])
    print('\nVerifying v'+args.version+' Signed MacOS\n')
    subprocess.check_call(['bin/gverify', '-v', '-d', '../gitian.sigs/', '-r', args.version+'-osx-signed', '../namecoin-core/contrib/gitian-descriptors/gitian-osx-signer.yml'])
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/namecoin/namecoin-core', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.chdir('namecoin-core')
    if args.pull:
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
        os.chdir('../gitian-builder/inputs/namecoin')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
