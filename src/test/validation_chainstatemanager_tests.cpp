...
struct SnapshotTestSetup : TestChain100Setup {
    ...
    /** Returns a vector of all coinbase transactions.  This includes the
        spendable genesis coinbase for Xaya.  */
    std::vector<CTransactionRef> GetCoinbaseTxns ()
    {
        auto res = m_coinbase_txns;
        res.push_back(::Params().GenesisBlock().vtx[0]);
        return res;
    }
    ...
    std::tuple<Chainstate*, Chainstate*> SetupSnapshot()
    {
        ...
        // Make some initial assertions about the contents of the chainstate.
        {
            ...
            /* In Xaya, the genesis coinbase is spendable and part of the UTXO
               set as well, but is not in the cache automatically (like the
               other blocks that were mined).  Put it in explicitly so that the
               rest of the test logic works.  */
            {
                const auto& genesisCoinbase = ::Params().GenesisBlock().vtx[0];
                const COutPoint genesisOut(genesisCoinbase->GetHash(), 0);
                BOOST_CHECK(ibd_coinscache.HaveCoin(genesisOut));
            }

            initial_size = ibd_coinscache.GetCacheSize();
            size_t total_coins{0};

            for (CTransactionRef& txn : GetCoinbaseTxns()) {
