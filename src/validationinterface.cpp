...
struct MainSignalsInstance {
    ...
    boost::signals2::signal<void (const std::shared_ptr<const CBlock> &, const std::vector<CTransactionRef>&)> BlockDisconnected;
...
void CMainSignals::BlockDisconnected(const std::shared_ptr<const CBlock> &pblock, const std::shared_ptr<const std::vector<CTransactionRef>> &pvNameConflicts) {
    m_internals->m_schedulerClient.AddToProcessQueue([pblock, pvNameConflicts, this] {
        m_internals->BlockDisconnected(pblock, *pvNameConflicts);
    });
}
