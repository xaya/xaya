...
BOOST_AUTO_TEST_CASE(check_max_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 BTC and 152x 0.33 BTC (~100.0 BTC total) unspent outputs
...

    {
        ...
        // The actor starts with 40x 0.625 BTC and 200x 0.25 BTC (75.0 BTC total) unspent outputs
        // Then tries to spend 49.5 BTC
        ...
        const auto result = select_coins(
            target, cs_params, cc, [&](CWallet& wallet) {
                ...
                for (int j = 0; j < 200; ++j) {
                    add_coin(available_coins, wallet, CAmount(0.25 * COIN), CFeeRate(0), 144, false, 0, true);
                }
                return available_coins;
            },
            chain);
        ...
        BOOST_CHECK(has_coin(result->GetInputSet(), CAmount(0.25 * COIN)));
    }

    {
        // Scenario 3:

        // The actor starts with 152x 0.33 BTC (49.995 BTC total) unspent outputs
        ...
        // No results
        // 152 inputs * 68 bytes = 10,336 bytes
        // 10,336 bytes * 4 = 41,344 weight, which is above the MAX_STANDARD_TX_WEIGHT of 40,000
        BOOST_CHECK(!result);
    }
}
