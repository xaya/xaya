                           ...
                           bool only_spendable)
{
    ...
    for (const auto& entry : wallet.mapWallet)
    {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if (output.scriptPubKey.IsPayToScriptHash(true) && solvable) {
        ...
        bool sign) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (withInput)
    {
        auto res = FindValueInNameInput(wallet, *withInput, withInputTx);
        if (!res)
            return util::Error{ErrorString(res)};
        nInputValue = res.value();
    }
    ...
    // Set the "noinput size" for coin selection based already on the tx with
    // the fixed input, in case that results in a larger estimate (so we
    // are on the safe side).
    if (withInput) {
        txNew.vin.emplace_back(withInput->prevout, CScript(), nSequence);
    }
    if (!coin_selection_params.m_subtract_fee_outputs) {
        const size_t template_size = CalculateMaximumSignedTxSize(CTransaction(txNew), &wallet, &coin_control).vsize;
        if (template_size > coin_selection_params.tx_noinputs_size)
            coin_selection_params.tx_noinputs_size = template_size;
    }
