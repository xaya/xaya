...
static void SetupCliArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to xayad). This changes the RPC endpoint used, e.g. http://127.0.0.1:8332/wallet/<walletname>", ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The xaya-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: xaya-cli [options] <command> [params]\n"
                "or:    xaya-cli [options] -named <command> [name=value]...\n"
                "or:    xaya-cli [options] help\n"
                "or:    xaya-cli [options] help <command>\n"
                "\n";
...
public:
        ...
        if (!args.empty()) {
            ...
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    ...
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: xaya-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: xaya-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    const std::string m_help_doc{
        ...
        + strprintf("> xaya-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
