...
static UniValue getaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The xaya address for account lookup.\n"
            "\nResult:\n"
...
static UniValue getaddressesbyaccount(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "  \"address\"         (string) a xaya address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n"
...
static UniValue sendfrom(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
        throw std::runtime_error(
            ...
            "                       the spend.\n"
            "2. \"toaddress\"         (string, required) The xaya address to send funds to.\n"
...
UniValue listtransactions(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("accounts")) {
        ...
        help_text = "listtransactions ( \"account\" count skip include_watchonly)\n"
            ...
            "    \"address\":\"address\",    (string) The xaya address of the transaction. Not present for \n"
...
static UniValue encryptwallet(const JSONRPCRequest& request)
{
    ...
    StartShutdown();
    return "wallet encrypted; Xaya server stopping, restart to run with encrypted wallet. The keypool has been flushed and a new HD seed was generated (if you are using HD). You need to make a new backup.";
}
