...
void BuyNamesPage::onRegisterNameAction()
{
    ...
    const QString err_msg = this->registerName(name, newValue, transferToAddress);
...
QString BuyNamesPage::registerName(const QString &name, const std::optional<QString> &value, const std::optional<QString> &transferTo) const
{
    ...
    LogPrintf ("wallet attempting name_register: name=%s\n", strName);
    ...
    UniValue res;
    try {
       res = walletModel->node().executeRpc("name_register", params, walletURI);
    }
    catch (const UniValue& e) {
        UniValue message = find_value(e, "message");
        std::string errorStr = message.get_str();
        LogPrintf ("name_register error: %s\n", errorStr);
