...
{
    auto& fakeHeader = starting_header.pow.initFakeHeader(starting_header);
    while (!starting_header.pow.checkProofOfWork(fakeHeader, Params().GetConsensus())) {
        ++fakeHeader.nNonce;
    }
}
        ...
        const uint256& merkle_root, const uint32_t nBits)
{
    ...
    while (headers.size() < count) {
        ...
        next_header.pow.setBits(nBits);
        next_header.pow.setCoreAlgo(PowAlgo::SHA256D);
...
BOOST_AUTO_TEST_CASE(headers_sync_state)
{
    ...
    /* We have the genesis block with Neoscrypt plus the rest
       with SHA-256d.  */
    arith_uint256 chain_work = (target_blocks - 1) * 2 + (2 << 10);

    /* Minimal regtest difficulty.  */
