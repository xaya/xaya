...
{
    return (GetTime<std::chrono::seconds>() - m_best_block_time.load()).count() / std::chrono::seconds{AvgTargetSpacing(m_chainparams.GetConsensus(), m_chainman.ActiveHeight())}.count();
}

bool PeerManagerImpl::CanDirectFetch()
{
    return m_chainman.ActiveChain().Tip()->Time() > NodeClock::now() - AvgTargetSpacing(m_chainparams.GetConsensus(), m_chainman.ActiveHeight()) * 20;
}
                ...
                if (MaybeSendGetHeaders(*pto, GetLocator(pindexStart), *peer)) {
                    ...
                    peer->m_headers_sync_timeout = current_time + HEADERS_DOWNLOAD_TIMEOUT_BASE +
                        (
                         ...
                         Ticks<std::chrono::seconds>(NodeClock::now() - m_chainman.m_best_header->Time()) / Ticks<std::chrono::seconds>(AvgTargetSpacing(consensusParams, m_chainman.m_best_header->nHeight))
                        );
                    nSyncStarted++;
                }
            }
