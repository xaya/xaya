...
BOOST_AUTO_TEST_CASE(blockmanager_flush_block_file)
{
    ...
    CBlock block3;
    block3.nVersion = 3;
    for (auto* b : {&block1, &block2, &block3})
        b->pow.initFakeHeader (*b);

    // They are 80 bytes header + 1 byte 0x00 for vtx length
    // PowData (with fake header) adds another 85 bytes.
    constexpr int TEST_BLOCK_SIZE{81 + 85};
