...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def test_external_inputs(self):
        ...
        # 65539 weight is beyond the limit in Xaya, so we are not testing that.
    ...
    def test_weight_limits(self):
        ...
        # 272 WU per input (273 when high-s); picking 148 inputs will exceed the max standard tx weight.
        rawtx = wallet.createrawtransaction([], [{wallet.getnewaddress(): 0.1 * 149}])

        # 1) Try to fund transaction only using the preset inputs (pick all 1472 inputs to cover the fee)
        input_weights = []
        for i in range(1, 150):  # skip first output as it is the parent tx change output
