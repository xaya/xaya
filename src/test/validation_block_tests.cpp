...
std::shared_ptr<CBlock> MinerTestingSetup::Block(const uint256& prev_hash)
{
    ...
    auto ptemplate = BlockAssembler(m_node.chainman->ActiveChainstate(), *m_node.mempool, Params()).CreateNewBlock(PowAlgo::NEOSCRYPT, CScript{} << i++ << OP_TRUE);
...
BOOST_AUTO_TEST_CASE(witness_commitment_index)
{
    CScript pubKey;
    pubKey << 1 << OP_TRUE;
    auto ptemplate = BlockAssembler(m_node.chainman->ActiveChainstate(), *m_node.mempool, Params()).CreateNewBlock(PowAlgo::NEOSCRYPT, pubKey);
