...
static UniValue generateBlocks(ChainstateManager& chainman, Mining& miner, const CScript& coinbase_output_script, int nGenerate, uint64_t nMaxTries, const UniValue& algoJson)
{
    const PowAlgo algo = powAlgoFromJson(algoJson);

    UniValue blockHashes(UniValue::VARR);
    while (nGenerate > 0 && !chainman.m_interrupt) {
        std::unique_ptr<BlockTemplate> block_template(miner.createNewBlock(algo, { .coinbase_output_script = coinbase_output_script }));
        CHECK_NONFATAL(block_template);

        std::shared_ptr<const CBlock> block_out;
        if (!GenerateBlock(chainman, block_template->getBlock(), nMaxTries, block_out, algo, /*process_new_block=*/true)) {
            break;
        }
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    return generateBlocks(chainman, miner, coinbase_output_script, num_blocks, max_tries, request.params[3]);
},
    };
}
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    ChainstateManager& chainman = EnsureChainman(node);
    {
        ...
        if (BlockValidationState state{TestBlockValidity(chainman.ActiveChainstate(), block, /*check_pow=*/false, /*check_bits=*/false, /*check_merkle_root=*/false)}; !state.IsValid()) {
    ...
    if (!GenerateBlock(chainman, std::move(block), max_tries, block_out, algo, process_new_block) || !block_out) {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    ...
    if (!request.params[0].isNull())
    {
        ...
        if (strMode == "proposal")
        {
            ...
            return BIP22ValidationResult(TestBlockValidity(chainman.ActiveChainstate(), block, /*check_pow=*/false, /*check_bits=*/false, /*check_merkle_root=*/true));
        }
    ...
    if (!pindexPrev || pindexPrev->GetBlockHash() != tip ||
        block_template->getBlock ().pow.getCoreAlgo() != algo ||
        (mempool.GetTransactionsUpdated() != nTransactionsUpdatedLast && GetTime() - time_start > 5))
    {
        ...
        // Create new block
        block_template = miner.createNewBlock(algo);
        CHECK_NONFATAL(block_template);
    ...
    arith_uint256 hashTarget = arith_uint256().SetCompact(block.pow.getBits());
    ...
    result.pushKV("bits", strprintf("%08x", block.pow.getBits()));
    result.pushKV("algo", PowAlgoToString(block.pow.getCoreAlgo()));
