...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        if (gArgs.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The namecoin-cli utility provides a command line interface to interact with a " CLIENT_NAME " RPC server.\n"
                ...
                "\n"
                "Usage: namecoin-cli [options] <command> [params]\n"
                "or:    namecoin-cli [options] -named <command> [name=value]...\n"
                "or:    namecoin-cli [options] help\n"
                "or:    namecoin-cli [options] help <command>\n"
                "\n";
...
public:
        ...
        if (!args.empty()) {
            ...
            if (args.size() > 1) {
                if (std::string_view s{args.at(1)}; n && (s == "o" || s == "outonly")) {
                    ...
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nFor more information, run: namecoin-cli -netinfo help", s));
                } else {
                    throw std::runtime_error(strprintf("invalid -netinfo outonly argument: %s\nThe outonly argument is only valid for a level greater than 0 (the first argument). For more information, run: namecoin-cli -netinfo help", s));
                }
            }
        }
        UniValue result(UniValue::VARR);
    ...
    const std::string m_help_doc{
        ...
        + strprintf("> namecoin-cli -netinfo %d outonly\n\n", NETINFO_MAX_LEVEL) +
